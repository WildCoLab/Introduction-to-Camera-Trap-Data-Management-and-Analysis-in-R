...
Dynamic exploration is key - do not leave it until the end. Also, plot from your dataframes. 



```{r}
# ADD THE DATA FRAMES HERE
```


## Standardised exploration script

In the Wildlife coexistance lab we use a standardised R script to generate summary information for camera trap projects. The most up to date script for exploring a single site can be found on our [GitHub page](https://github.com/WildCoLab/SingleSiteExploration).

### The output
```{r}
## ADD THE MARKDOWN CODE HERE
```
...
#Load Packages
list.of.packages <- c("leaflet", "dplyr", "colortools", "kriging", "corrplot", "lubridate", "kableExtra")
