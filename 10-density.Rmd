...
## Unmarked animals
However, it is not possible to identify individual animals for the vast majority of species detected by camera traps. There are a growing number of analysis frameworks for unmarked animals, including:  
...
r = radius of camera detection zone [Could use field trials - can vary station to station]
...
To use `spaceNtime` we need a dataframe with the 'placename' labelled as 'cam' (assuming there is one camera in each location), `timestamp` renames to `datetime` and `event_groupsize` relabeled as 'count'.
...
Lots of great examples in here:
