...
We can extract elevation from Amazon Web Service Terrain Tiles... available globally!

```{r ch4_23b, class.source="Rmain", warning=F, eval=T}
...
```
The `src` option specifies the sources of the DEM data. We use `aws` Amazon Web Service Terrain Tiles.
...
```{r ch4_46b, class.source="Rmain", eval=T, include=F}
# Specify your own credentials:
log <- mf_login(credentials = c("cameratrap_course", "CMI_Revelstoke_2025")) # set your own EOSDIS username and password
```

For `modisfast` to work, we need to provide a sf polygon with an "id" column. We can do this easily using the `aoi` object we created earlier"
...
We then use these to specify the location of the data and download it to our computers. 

```{r ch4_47b, class.source="Rmain", message=F, warning=F, echo=T, eval=F}
...
```{r ch4_47c, message=F, warning=F, echo=F, include=F}
## Get the URLs of the data
urls <- mf_get_url(
  collection = collection,
  variables = variables,
...
Add the data to your locations covariates dataframe and check the output:

```{r ch4_54, class.source="Rmain"}
