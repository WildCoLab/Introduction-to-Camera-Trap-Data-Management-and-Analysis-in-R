...
list.of.packages <- c("kableExtra", "tidyr", "ggplot2", "gridExtra", "dplyr", "unmarked", "lubridate", "tibble", "sf", "gfcanalysis",
                      "spOccupancy")
...
tmp_week  <- week_obs[, c("placename", "date", "days", focal_sp)]

# subset to 2019
tmp_week <- tmp_week[substr(tmp_week$date,1,4)==2019,]


###########################
# Create the Y data matrix 
y_dat <- tmp_week[,c("placename", "date", focal_sp)] %>%
            pivot_wider(names_from = date, values_from = focal_sp)

# Convert it to a matirx - but only keeo the date values
...
First we must make an unmarked dataframe - a data structure which speaks to the unmarked package architecture:
...
Unmarked models benefit from data standardization - it can help the solving algorythmns converge,
...
un_dat <- unmarkedFrameOccu(y = y_mat,
...
Le's fit a couple of other models!

First with a continuous covariate on the occupancy probability, then a catagorical one too:
...
We can observe the relationship between our covariates and our occupancy probabilities through the use of a dummy dataframes (which we will call `new_dat`). A dummy dataframe is essential just a dataframe built up of `dummy` data - which lies within the upper and lower limits of the covariates we already have. We wouldnt want to extrapolate beyond our data! We can then plot the results:

```{r}
# Generate new data to predict from 
new_dat <- cbind(expand.grid(
                  z.line_of_sight_m=seq(min(z_locs$z.line_of_sight_m),max(z_locs$z.line_of_sight_m), length.out=25) 
                ))
...
p1 <- ggplot(new_dat, aes(x = z.line_of_sight_m, y = Predicted)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5, linetype = "dashed") +
  geom_path(size = 1) +
  labs(x = "Line of sight", y = "Occupancy probability") +
  theme_classic() +
