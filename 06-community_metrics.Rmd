...
Applying the iNEXT functions to camera trap data is perhaps simplest using 'abundance' function, this represents the frequency with which each species is detected. The abundance data model requires a string of abundance frequencies contained within a list. 

```{r ch6_5, echo=F, message=F, warning=F, out.width="75%"}
knitr::include_graphics("images/community_metrics/data_format.PNG")
```

We can create this format from the `...total_observations` dataset:
...
Once you have created your list, we can run a basic `iNEXT` analysis and create a graphs of the results. Below we show a basic `iNEXT` call with some of its default arguments.
...
The example above is useful, but in camera trap projects we typically do not think about our survey effort in terms of the number of individuals we catch, rather the number of camera stations we deploy on the landscape or the units of time they are active (e.g. camera days). 

Performing our species accumulation curves using survey location, rather than individuals encountered, allows us to determine if we have enough survey locations in a given strata to detect all of the species present. Repeating the analyses using camera days would also give insight into whether we need for survey effort in a given location. 
...
```{r ch6_17, echo=F, message=F, warning=F,  out.width="100%"}
...
Lets build this example for survey stations using our data:

```{r ch6_18, class.source="Rmain"}
...
**Simple:** Repeat the comparison for `NetReg` and `HumanUse` feature types.
...
# To keep things simple, we will subset to HumanUse and Offline (as above)
dat <- dat[dat$feature_type %in% c("HumanUse", "Offline"),]
...
nmds = metaMDS(m_counts, distance = "bray")
nmds
...
Repeat the comparison for `NetReg` and `HumanUse` feature types.
