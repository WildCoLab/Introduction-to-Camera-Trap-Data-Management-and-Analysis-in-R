...
list.of.packages <- c("kableExtra", "tidyr", "ggplot2", "gridExtra", "lme4", "dplyr", "Hmsc", "jtools", "lubridate")
...
This 'detection rate' is thought to reflect the habitat use of a species at a given location. Extreme care should be taken if you want to equate use with abundance or density - something we discuss a litle more in the [density chapter](#density). 
...
The most common way to analyse habitat-use data is through linear models. Linear models typically relate a continuous response variable - in our case capture rate - to a set of one or more discrete or continuous predictor variables. In this simple example we will explore the relationship between the capture rate of a species with the catagorical 'feature_type' variable and the continuous `line_of_sight_m` variables. 

There are a variety if different frameworks to fit and compare different linear models to address a host of different hypotheses, but if you are just starting out you should investiagte two widely used packages:
...
We will start by analyzing a frequentist linear model with a single observation for each camera location.
...
# Convert to catagorical factors
locs <- locs %>% 
...
Add our covariates to our capture rate dataframe: 

```{r}
library(dplyr)
mod_dat <- left_join(total_cr, locs)
```
...
effect_plot(lm_cat,                  # The model object
            ...
            partial.residuals = T,   # Show the residual variation -after sccounting for fixed effects  
...
```{r}
plot(mod_dat$Odocoileus.virginianus~mod_dat$line_of_sight_m,
        las=1,
...
```{r}
# model results <- lm( Y data ~ x Data, data= dataframe source)
lm_con <- lm(Odocoileus.virginianus ~ line_of_sight_m, data = mod_dat)
```
...
Here the effect represents the gradient of the relationship between `line_of_sight_m` and the habitat use of white-tailed deer. The effect is negative, and the p-value is below the arbitrary 0.05 threshold, whih suggests it my be an important predictor of white-tailed deer habitat use. 

It will make more sense if we plot it - again using `jtools`

```{r}
effect_plot(lm_con,                  # The model object
            pred = line_of_sight_m,  # The variable you want to predict
            interval = TRUE,         # Whether you want confidence intervals (default = 0.95)
            partial.residuals = T,   # Show the residual variation -after sccounting for fixed effects  
            y.label = "Habitat use") # Change the y axis label
```
...
```{r}
glmm_cat <- glmer(Odocoileus.virginianus ~ 
                    feature_type + season + offset(log(days)) + (1|placename) , data=mod_dat, family="poisson")
```
...
As with our simple linear model, the mixed effects model also suggests a difference between the different `feature_type` strata for white-tailed deer habitat -se. 
...
Which suggests habitat use is slightly lower in winter then in summer. 
...
### Advance mixed-model predictions
...
**A note of caution** In experimenting with single species models you may have realized it can sometimes be hard to build a sensible and robust model. Now do this for >10 species in the **same model**, and the potnetial to get silly results increases. Tread carefully!
...
```{r}
Xdat <- left_join(monthly_obs[c("placename", "date", "days")], locs)
...
# Model specification
mod <- Hmsc(Y = Y, XData = Xdat[,c("line_of_sight_m", "water_depth_m", "days")], 
         XFormula = ~line_of_sight_m + water_depth_m + log(days),
...
par(mar=c(8,12,1,1))
plotBeta(out, post = postBeta, param = "Support", supportLevel = 0)
```
