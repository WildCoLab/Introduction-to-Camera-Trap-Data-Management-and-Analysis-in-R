...
Now things start to get really interesting - we are getting closer to analysing our data. Before we get into building any models however, we must thoroughly explore our data. We want to ask questions like?
...
```{r, message=F, warning=F, class.source="Rmain"}
...
# We can now sum the presenc/absences and divide by the number of survey locations
tmp <- long_bin %>% 
  group_by(sp) %>% 
...
```{r, warning=F, message=F, class.source="Rmain"}
...
```{r, class.source="Rmain"}
# We first need to convert the date colum to a date object
...
Blue colours = positive correlation ->  at locations where you have high counts of one species, you also have high counts of the paired species. 

Red colours = negative correlation -> at locations where you have high counts of one species, then you are likely to have low counts of the species pair (or vice-versa).

We implement a more nuanced form of this data analysis in the [interactions chapter](#interactions). To make this plut we use the `total_obs` dataframe.

```{r, class.source="Rmain"}
# Reset the plot parameters
par(mfrow=c(1,1))

# Pull the datat for each of the species from 
...
corrplot(M, method="color", 
         type="upper", 
         order="hclust",
         # addCoef.col = "black", # We supress the coefs to make a cleaner plot
...
The great thing about `ggplot` is that it is easy to add trendlines:

```{r, message=F, warning=F, class.source="Rinfo"}
ggplot(data=total_obs, 
       aes(x=line_of_sight_m, y=Alces.alces)) + 
       geom_point() +                   # Specify a scatter plot
       theme_classic() +  
...
For catagorical predictors boxplots are very useful!
...
- Wolves (**Canis lupus**) select locations with longer line of sight
- Caribou (**Rangifer tarandus**) select locations where the water table is on the surface
- White-tailed deer (**Odocoileus virginianus**) select locations with higher vegetation productivity (e.g. higher NDVI)
